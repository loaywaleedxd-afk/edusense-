...
import aiosqlite, os, json, smtplib, ssl
...
async def _compute_student_risk(db, student_id: str) -> dict:
    ...
    # ── Factor 2: Grades ─────────────────────────────────────────────────────
    grades = await (await db.execute(
        "SELECT AVG(grade) as avg FROM grades WHERE student_id=?", (student_id,)
    )).fetchone()
    ...
    enrolled = await (await db.execute(
        "SELECT COUNT(*) as n FROM course_enrollments WHERE student_id=?", (student_id,)
    )).fetchone()
    n_courses = enrolled["n"] if enrolled else 0

    assign_total = await (await db.execute(
        "SELECT COUNT(*) as n FROM assignments WHERE course_id IN "
        "(SELECT course_id FROM course_enrollments WHERE student_id=?)",
        (student_id,)
    )).fetchone()
    assign_done = await (await db.execute(
        "SELECT COUNT(*) as n FROM submissions WHERE student_id=? AND grade IS NOT NULL",
        (student_id,)
    )).fetchone()
...
def _send_alert_email(to: str, student_name: str, risk_level: str,
    ...
    subject = f"[EduSense] {emoji} {risk_level.upper()} Risk Alert — {student_name}"
...
<div style="background:#fff;padding:28px">
  <div style="background:{color}15;border-left:4px solid {color};padding:16px 20px;border-radius:8px;margin-bottom:24px">
    <div style="font-size:22px;font-weight:900;color:{color}">{emoji} {risk_level.upper()} RISK</div>
  ...
  <div style="text-align:center;margin-top:20px">
    <a href="http://localhost:5173" style="background:#1a237e;color:#fff;padding:12px 32px;border-radius:8px;text-decoration:none;font-weight:700;font-size:13px">
...
@router.post("/assess")
async def run_assessment(payload: dict = Depends(require_role("doctor", "admin"))):
    """Re-calculate risk scores for all students and save to DB."""
    async with aiosqlite.connect(DB) as db:
        db.row_factory = aiosqlite.Row
        students = await (await db.execute("SELECT student_id FROM students")).fetchall()
...
async def student_risk(student_id: str, payload: dict = Depends(require_auth)):
    """Get the latest risk assessment for a specific student."""
    async with aiosqlite.connect(DB) as db:
        db.row_factory = aiosqlite.Row
        row = await (await db.execute(
            "SELECT * FROM at_risk_assessments WHERE student_id=?", (student_id,)
        )).fetchone()
    if not row:
...
async def notify_at_risk(
    ...
    async with aiosqlite.connect(DB) as db:
        ...
        # Get parent email (users table with role='parent' linked to student)
        ...
        # Get advisor email (first doctor in student's courses)
        advisor = await (await db.execute("""
        ...
        if notify_advisor and advisor and advisor["email"]:
            ...
            await db.execute(
                "UPDATE at_risk_assessments SET advisor_notified=1 WHERE student_id=?",
                (student_id,)
            )
        ...
        if notify_parent and parent and parent["email"]:
            ...
            await db.execute(
                "UPDATE at_risk_assessments SET parent_notified=1 WHERE student_id=?",
                (student_id,)
            )
