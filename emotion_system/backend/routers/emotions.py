"""
Emotions router — record & retrieve emotion data
"""
...
@router.post("/record")
async def record_emotion(rec: EmotionRecord):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            """INSERT INTO emotion_records
               (student_id,lecture_id,emotion,confidence,attention_score,engagement_score)
...
async def get_lecture_emotions(lecture_id: str):
    async with aiosqlite.connect(DB_PATH) as db:
...
async def emotion_distribution(lecture_id: str):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute(
...
async def emotion_trends(lecture_id: str):
    """Time-based emotional trends aggregated by minute."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute(
...
async def student_emotions(student_id: str, limit: int = 100):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute(
